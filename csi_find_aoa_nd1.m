...
function [tofs,rads,Pmu] = csi_find_aoa_nd1(csi_frame, e_csi)
...
%     end
    ...
    % algorithm newdream
    smoothed_csi = zeros(15,22);
    for row=1:5
        smoothed_csi(row,:) = csi_matrix(1,row*2-1:row*2+20); 
        smoothed_csi(row+5,:) = csi_matrix(2,row*2-1:row*2+20); 
        smoothed_csi(row+10,:) = csi_matrix(3,row*2-1:row*2+20); 
    end
    ...
    degrees = -90:0.5:90; % linespace
    [~,deg_tot] = size(degrees);
    tofs = 5e-9:1e-9:300e-9;
%     sig_tof = 6;
    [~,tof_tot] = size(tofs);
