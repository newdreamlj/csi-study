...
function [tofs,rads,Pmu] = csi_find_aoa_multipkt(csi_frame,e_csi_pkts,num_of_pkt)

narginchk(3,3)

x_sc = 1:57;
smoothed_csi_all = zeros(30,86*num_of_pkt);

for pkt=1:num_of_pkt
    csi_matrix = squeeze(e_csi_pkts(pkt,:,:)); % 
    amp = abs(csi_matrix);
    fi = angle(csi_matrix);
    fii = fi;
...
% 
% % figure(5)
% % plot(x_sc,fii(:,1)' - polyval(a,x_sc,1))
% % hold on
% % plot(x_sc,fii(2,:)' - polyval(a,x_sc,1))
% % plot(x_sc,fii(3,:)' - polyval(a,x_sc,1))
% % hold off
...
%% Smoothed CSI Matrix
    ...
    smoothed_csi_all(:,((pkt-1)*86+1):(pkt*86)) = smoothed_csi;
end
...
MUSIC_S = smoothed_csi_all * smoothed_csi_all';
...
% [aoa_v,aoa_k] = max(Pmu);
% [aoa_vv,aoa_kk] = max(aoa_v);
% 
% aoa = degrees(aoa_k(aoa_kk));
rads = degrees'*pi/180;
% pmu = Pmu(:,sig_tof);
